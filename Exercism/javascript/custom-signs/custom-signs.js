...
export function costOf(sign, currency) {
  const basePrice = 20;
  const pricePerCharacter = 2;
  const characterCost = sign.length * pricePerCharacter;
  const totalPrice = basePrice + characterCost;

  return `Your sign costs ${totalPrice}.00 ${currency}.`;
}
